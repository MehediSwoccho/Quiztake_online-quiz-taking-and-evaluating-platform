...
export const ClassDetailsPage = () => {
  ...
  const handleViewQuiz = (quizId, status) => {
    ...
    if (status === 'active' && !isTeacher) {
      navigate(`/quiz/${quizId}`, { state: { action: 'start', classId } });
    } else {
      navigate(`/quiz/${quizId}`, { state: { classId } });
    }
  }
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white shadow rounded-lg p-6 mb-6">
        ...
        {!isTeacher && (
          <div className="space-y-4">
            ...
            {/* Consultation section for students */}
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-gray-600 text-sm">Request a consultation with your teacher for help and guidance</p>
                </div>
                <button
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-gray-600 text-sm">
                    {classDetails?.studentRating 
                      ? `Your rating: ${classDetails.studentRating}` 
                      : 'Share your feedback about the teacher'
                    }
                  </p>
                </div>
                <button
                ...
                >
                  ...
                  {classDetails?.studentRating ? 'Update Rating' : 'Rate Teacher'}
                </button>
              </div>
            </div>
          </div>
      ...
      {/* Feedback Section */}
      <FeedbackSection classId={classId} />
      
      {/* Rating Statistics for Teachers */}
      {isClassTeacher && classDetails?.ratingStats && (
        <div className="bg-white shadow rounded-lg p-6 mb-6">
          <h2 className="text-xl font-semibold mb-4">Student Ratings</h2>
          {classDetails.ratingStats.total === 0 ? (
            <p className="text-gray-500">No ratings yet from students.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-center">
                ...
                <div className="text-2xl font-bold text-green-600">{classDetails.ratingStats.good}</div>
                <div className="text-sm text-gray-600">Good Ratings</div>
                <div className="text-xs text-gray-500 mt-1">
                  {classDetails.ratingStats.total > 0 ? Math.round((classDetails.ratingStats.good / classDetails.ratingStats.total) * 100) : 0}%
                </div>
              </div>
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-center">
                ...
                <div className="text-2xl font-bold text-yellow-600">{classDetails.ratingStats.average}</div>
                <div className="text-sm text-gray-600">Average Ratings</div>
                <div className="text-xs text-gray-500 mt-1">
                  {classDetails.ratingStats.total > 0 ? Math.round((classDetails.ratingStats.average / classDetails.ratingStats.total) * 100) : 0}%
                </div>
              </div>
              ...
              <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-center">
                ...
                <div className="text-2xl font-bold text-red-600">{classDetails.ratingStats.bad}</div>
                <div className="text-sm text-gray-600">Bad Ratings</div>
                <div className="text-xs text-gray-500 mt-1">
                  {classDetails.ratingStats.total > 0 ? Math.round((classDetails.ratingStats.bad / classDetails.ratingStats.total) * 100) : 0}%
                </div>
              </div>
            </div>
          )}
          <div className="mt-4 text-center text-sm text-gray-600">
            Total Ratings: {classDetails.ratingStats.total}
          </div>
        </div>
      )}
      ...
      <RatingModal
        ...
        classId={classId}
        className={classDetails?.name}
        currentRating={classDetails?.studentRating}
      />
    </div>
  )
}