...
export const QuizPage = () => {
  ...
  // timers for debounced short-answer autosave per question
  const saveTimersRef = useRef({})
  // guard to prevent duplicate start calls
  ...
  // Prefer classId from loaded quiz; fallback to classId passed via navigation state
  const classIdForBack = currentQuiz?.classId || location.state?.classId
   
  useEffect(() => {
    const loadQuiz = async () => {
      if (!quizId) return;

      const starting = location.state?.action === 'start';
      if (starting && !hasStartedRef.current) {
        try {
          hasStartedRef.current = true;
          await startQuiz(quizId);
          return; // Skip further fetches to reduce spinner time
        } catch (error) {
          console.error("Error starting quiz immediately:", error);
        }
      }

      try {
        // Fetch quiz details when not starting immediately
        const result = await fetchQuizDetails(quizId);
        if (result.success) {
          const quizStatus = getQuizStatus();

          if (!isTeacher) {
            if (quizStatus === 'upcoming') return;
            // Only fetch existing submission if not starting now
            if (!starting) {
              try {
  ...
  }, [quizId, location.state, isTeacher, startQuiz, fetchQuizDetails, fetchSubmission, fetchQuizSubmissions, clearQuizState]);

  // Clear any pending short-answer autosave timers on unmount
  useEffect(() => {
    return () => {
  ...
  // Fallback: if details already loaded and we navigated with start, trigger start once
  useEffect(() => {
    if (location.state?.action === 'start' && quizId && currentQuiz && !hasStartedRef.current) {
      hasStartedRef.current = true;
      handleStartQuiz();
    }
  }, [location.state, quizId, currentQuiz])

  useEffect(() => {
    if (!currentQuiz || !quizSubmission || quizSubmission.status !== "in-progress" && quizSubmission.status !== "started") {
      return
    }
    ...
    const initialRemaining = Math.max(0, quizEndForStudent - now)
    setTimeLeft(initialRemaining)
    if (initialRemaining === 0) {
      // Time already elapsed; complete without creating an interval
      handleCompleteQuiz()
      return
    }
  ...
  useEffect(() => {
    if (!quizId || !isTeacher) return;
    const params = new URLSearchParams(location.search);
    const studentId = params.get('studentId');
    if (!studentId) return;

    setSelectedStudentId(studentId);
    fetchSubmission(quizId, studentId).catch(() => {
      // ignore errors here; the page already handles error state
    });
  }, [quizId, isTeacher, location.search, fetchSubmission]);
  ...
  const handleAnswer = async (questionId, answer) => {
    const status = quizSubmission?.status
    if (isSubmitting || !(status === "in-progress" || status === "started")) return

    // Determine question type first
    const question = currentQuiz?.questions?.find(q => q._id === questionId || q.id === questionId)
    const isShort = question?.questionType === 'short-answer'

    if (isShort) {
      // Update only local buffer for smooth typing
      setTypedAnswers(prev => ({ ...prev, [questionId]: answer }))

      // Debounce network submit and then reflect into answers
      ...
      saveTimersRef.current[questionId] = setTimeout(async () => {
        try {
          ...
          // reflect into lightweight answers map so nav shows answered
          setAnswers(prev => ({ ...prev, [questionId]: answer }))
        } catch (err) {
          setQuizError('Failed to autosave your answer')
        }
      }, 600)
      return
    }

    // Non-text: update answer immediately and submit
    setAnswers(prev => ({ ...prev, [questionId]: answer }))
    try {
      await submitAnswer(quizSubmission._id, questionId, answer)
    } catch (err) {
  ...
  // Flush any pending short-answer debounce immediately (e.g., on blur or navigation)
  const flushShortAnswer = async (questionId) => {
    const status = quizSubmission?.status
    if (!(status === "in-progress" || status === "started")) return
    if (!typedAnswers || !(questionId in typedAnswers)) return
    const answer = typedAnswers[questionId]
    if (saveTimersRef.current[questionId]) {
      clearTimeout(saveTimersRef.current[questionId])
    ...
    try {
      ...
      setQuizError('Failed to save your answer')
    }
  }
  ...
  const handleCompleteQuiz = async () => {
    if (!quizSubmission || quizSubmission.status !== "in-progress" && quizSubmission.status !== "started") return
  ...
  const handleBack = () => {
    if (classIdForBack) navigate(`/classes/${classIdForBack}`)
  ...
  const canTakeQuiz = !isTeacher && 
                       quizStatus === "active" && 
                       (quizSubmission?.status === "in-progress" || quizSubmission?.status === "started")
  
  if ((quizSubmission && quizSubmission.status !== "in-progress" && quizSubmission.status !== "started") || viewingResults) {
        ...
        <div className="bg-white shadow rounded-lg p-6">
          ...
          <div className="space-y-4">
            ...
            {currentQuestion.questionType === 'short-answer' && (
              <div>
                <textarea
                  value={(typedAnswers[currentQuestion._id] ?? currentAnswer ?? '')}
                  onChange={(e) => handleAnswer(currentQuestion._id, e.target.value)}
                  onBlur={() => flushShortAnswer(currentQuestion._id)}
        ...
        <div className="bg-white shadow rounded-lg p-6 mt-6">
          ...
          <div className="flex flex-wrap gap-2">
            {currentQuiz.questions.map((question, i) => {
              const ta = typedAnswers[question._id]
              const hasAnswer = (ta !== undefined && ta !== '') || answers[question._id] !== undefined
