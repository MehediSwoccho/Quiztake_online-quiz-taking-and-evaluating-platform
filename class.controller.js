...
// Enroll in a class using enrollment code for students
...
export const getClassDetails = async (req, res) => {
  try {
    ...
    // Add rating statistics for teachers
    ...
    if (isTeacher) {
      ...
      ratings.forEach(rating => {
        ...
        ratingStats.total += rating.count;
      });
    }

    // Add student's rating for this class if they are a student
    let studentRating = null;
    if (isStudent) {
      studentRating = await Rating.findOne({
        student: user._id,
        class: classId
      });
    }
    ...
    return res.status(200).json({
      ...
      ratingStats,
      studentRating: studentRating ? studentRating.rating : null
    });
  } catch (error) {
